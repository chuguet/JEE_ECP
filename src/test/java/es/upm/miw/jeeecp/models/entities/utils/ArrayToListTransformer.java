...
public class ArrayToListTransformer {
    ...
    public static Iterator<TemaEntity> transform(Integer[] id, String[] nombre, String[] pregunta,
            String[][][] votos) {
        ...
        for (int i = 0; i < id.length; i++) {
            if (votos[i] != null) {
                ...
                for (int j = 0; j < votos[i].length; j++) {
                    votoEntity = new VotoEntity(Integer.parseInt(votos[i][j][0]), votos[i][j][1],
            ...
            temasEntities.add(new TemaEntity(id[i], nombre[i], pregunta[i], votosEntities));
        }
